...
function pt_wc_rb_process_post() {
	...
	if ( isset( $_POST['pt_wc_rb_search_local'] ) ) {
		...
		//echo 'pt_wc_rb_get_number_details( ' . $phone_number .' )<pre>' . print_r( pt_wc_rb_get_number_details( $phone_number, true ), 1 ) . '</pre>';
		//echo 'pt_wc_rb_reserve_number( ' . $phone_number .' )<pre>' . print_r( pt_wc_rb_reserve_number( $phone_number ), 1 ) . '</pre>';
		//echo 'pt_wc_rb_release_number( ' . $phone_number .' )<pre>' . print_r( pt_wc_rb_release_number( $phone_number, true ), 1 ) . '</pre>';
		//echo 'pt_wc_rb_order_number( ' . $phone_number .' )<pre>' . print_r( pt_wc_rb_order_number( $phone_number ), 1 ) . '</pre>';
	...
	if ( ! $details ) {
		...
		*/
		...
		if ( WC()->cart->add_to_cart( $product_id ) ) {
			...
			wc_add_notice( 'Phone number ' . $phone_number . ' added to cart' );
			wp_safe_redirect( wc_get_cart_url() );
			exit;

		} else {

			wc_add_notice( 'Error adding phone number to cart' );
			get_template_part( 'local-phone-number-result' );
			exit;
...
function pt_wc_rb_reserve_number( $number ) {
	...
	if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
		...
		/**
		$message = esc_html__( 'Error reserving number: ', 'pt-wc-ringboost' ) .  ' 
								<pre> ' . print_r( $response, 1 ) . '</pre>
								<pre> ' . print_r( $args, 1 ) . '</pre>';
		wp_die( $message );/**/
...
function pt_wc_rb_plugins_loaded() {
	...
	//pt_wc_rb_maybe_order_phone_numbers( 343 );


	// Display a Settings link on the main WP Plugins page
	//add_filter( 'plugin_action_links', 'pt_wc_rb_action_links', 10, 2 );

	// our admin functions
	//add_action( 'admin_menu', 'pt_wc_rb_admin_menu', 10 );

	// i18n
	load_plugin_textdomain( 'pt-wc-ringboost', false, '/pt-wc-ringboost/languages' );


	function pt_wc_rb_admin_menu() {
		...
		if ( current_user_can( 'manage_woocommerce' ) ) {
			...
			add_submenu_page( 'woocommerce',
				 ...
				 __( 'Indica Tudo', 'pt-wc-ringboost' ) ,
				 'manage_woocommerce',
				 'pt_wc_rb_ringboot_integration',
				 'pt_wc_rb_show_menu_page');
		}
	}


	function pt_wc_rb_show_menu_page() {
		...
		<div class="wrap woocommerce">
			...
			<h2 class="nav-tab-wrapper"> 
				...
				<?php esc_html_e( 'Coupons', 'pt-wc-ringboost' ); ?></a>
		...
		<?php
			...
			switch ( $tab ) {
				...
				case 'api':
					pt_wc_rb_display_api_tab();
				break;

				case 'coupons':
					pt_wc_rb_display_coupons_tab();
				break;

				case 'log':
					pt_wc_rb_display_log_tab();
				break;
	...
	function pt_wc_rb_display_api_tab() {

		//delete_option( 'coupons_indicatudo');
		//pt_wc_rb_process_api_check();

		//$order = wc_get_order( 90 );
		//pt_wc_rb_process_venda( $order );
		...
		?>
			...
			<div class="panel woocommerce_options_panel">
				<h1>Login details: <pre><?php print_r( pt_wc_rb_get_api_details() ) ?></pre></h1>
			...
			<?php
				...
				if ( wp_next_scheduled( 'pt_wc_rb_api_check' ) ) {
					...
					<h2><?php printf( esc_html__( 'Next API update %s', 'pt-wc-ringboost'), pt_wc_rb_better_human_time_diff( wp_next_scheduled( 'pt_wc_rb_api_check' ) ) ); ?></h2>
			...
			<h3> To do list</h3>
			<ul>
				<li>Cron manager: <a href="<?php echo admin_url( 'tools.php?page=advanced-cron-manager' );?>">pt_wc_rb_api_check</a></li>
	...
	function pt_wc_rb_display_coupons_tab() {
	...
	function pt_wc_rb_display_log_tab() {
			...
			<div class="panel woocommerce_options_panel">
				...
				<table class="widefat">
					...
					<tbody>
						...
						foreach ( pt_wc_rb_get_log() as $event ) {
							...
							<tr <?php echo $class ?>>
								<td><?php echo pt_wc_rb_nice_time( $event[0] ); ?></td>
	...
	function pt_wc_rb_better_human_time_diff( $from, $to = '', $limit = 3 ) {
	...
	function pt_wc_rb_action_links( $links, $file ) {

		if ( plugin_basename( __FILE__ ) == $file ) {

			$url = get_admin_url() . 'admin.php?page=pt_wc_rb_ringboot_integration&tab=api';

			$pt_wc_rb_settings_link = '<a href="' . esc_url( $url ) . '">' . esc_html__( 'Settings', 'pt-wc-ringboost' ) . '</a>';

			// make the 'Settings' link appear first
			array_unshift( $links, $pt_wc_rb_settings_link );
		}
	...
	function pt_wc_rb_nice_time( $time, $args = false ) {
	...
	function pt_wc_rb_log( $event ) {
		$current_user = wp_get_current_user();
		$current_user_id = $current_user->ID;

		$log = get_option( 'pt_wc_rb_log' );
		...
		return update_option( 'pt_wc_rb_log', $log );
	}


	function pt_wc_rb_get_log() {
		$log = get_option( 'pt_wc_rb_log' );
		...
		if ( ! is_array( $log ) ) {
			...
			$time            = time() + $time_difference;
			array_push( $log, array( $time, __( 'Log Started.', 'pt-wc-ringboost' ), $current_user_id ) );
			update_option( 'pt_wc_rb_log', $log );
		}
		return array_reverse( get_option( 'pt_wc_rb_log' ) );
	}


	function pt_wc_rb_delete_log() {
		$current_user    = wp_get_current_user();
		$current_user_id = $current_user->ID;
		$log             = array();
		$time_difference = get_option( 'gmt_offset' ) * 3600;
	...
	function pt_wc_rb_clear_old_log_entries() {
		$log = get_option( 'pt_wc_rb_log' );
		if ( is_array( $log ) ) {
